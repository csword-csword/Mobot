...
function Callout({ data, align }: { data: CalloutData; align: 'left' | 'right' }) {
  return (
    ...
    >
      <h3 className="text-sm font-bold text-[#1d4ed8] mb-1">{data.title}</h3>
      <p className="text-slate-700 text-xs sm:text-sm leading-relaxed">{data.body}</p>
      <p className="text-slate-400 text-xs leading-relaxed mt-1">{data.sim}</p>
    </div>
  );
}
...
function PhoneIllustration() {
  return (
    <svg
      viewBox="0 0 200 400"
      className="hidden md:block row-span-3 self-stretch w-32 mx-auto h-full text-[#1d4ed8]"
      aria-hidden="true"
    >
      {/* frame */}
      <rect x="4" y="4" width="192" height="392" rx="34" fill="rgba(15,23,42,0.02)" stroke="currentColor" strokeOpacity="0.3" strokeWidth="3" />
      {/* screen inset */}
  ...
  return (
    <section className="py-24 px-6" aria-label="Anatomy of an escaped defect">
      <div className="mx-auto max-w-[64rem]">
        <p className="eyebrow text-xs mb-4 text-center">The Anatomy</p>
        <h2 className="text-3xl font-bold text-[#0a2540] mb-2 text-center">Anatomy of an Escaped Defect</h2>
        <p className="text-slate-500 text-sm text-center mb-12 max-w-[32rem] mx-auto">
