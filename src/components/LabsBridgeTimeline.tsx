  ...
  return (
    ...
    >
      <div className="relative flex justify-between items-start">
        <div className="absolute top-[7px] left-[8%] right-[8%] h-px bg-white/10" />
        {nodes.map((n) => (
          <div key={n.label} className="relative z-10 flex flex-col items-center text-center w-1/3 px-2">
            <span
              className={
                n.future
                  ? 'w-3.5 h-3.5 rounded-full border-2 border-[#3da6fc] bg-[#0B0F14] mb-4'
                  : 'w-3.5 h-3.5 rounded-full bg-[#3da6fc] mb-4'
              }
            />
            <h3 className="font-bold text-sm sm:text-base mb-1">{n.label}</h3>
            <p className="text-white/50 text-xs sm:text-sm">{n.sub}</p>
          </div>
        ))}
      </div>
      ...
      <Link
        href="/schedule-demo"
        className="mt-10 flex items-center justify-center gap-2 rounded-full bg-[#2f87c8]/10 border border-[#3da6fc]/40
                   text-[#3da6fc] px-6 py-3 text-sm font-semibold hover:bg-[#2f87c8]/20 transition-colors text-center"
      >
