                ...
                <div className="flex-1 flex flex-col items-center text-center px-1">
                  <span
                    className={
                      ...
                      (h.warn ? 'border-red-600/60 text-red-600' : 'border-[#2f87c8]/60 text-[#2f87c8]')
                    }
                  >
                    {i + 1}
                  </span>
                  <h3 className="text-sm font-bold mb-2">{h.label}</h3>
                  <p className={'text-xs leading-relaxed ' + (h.warn ? 'text-red-600/80' : 'text-black/50')}>
                    {h.detail}
                  </p>
                </div>
                {i < hops.length - 1 && (
                  <div className="pt-3 text-black/20 shrink-0">&rarr;</div>
                )}
              </Fragment>
            ))}
          </div>
