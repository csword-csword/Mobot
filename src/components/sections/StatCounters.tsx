...
function StatItem({ target, suffix, label, active }: { target: number; suffix: string; label: string; active: boolean }) {
  ...
  return (
    <div className="text-center px-4 md:first:pl-0 md:last:pr-0">
      ...
      <span className="block text-white/40 text-xs uppercase tracking-[0.2em] mt-3">{label}</span>
    </div>
  );
}
...
export default function StatCounters() {
  ...
  return (
    <section className="py-24 px-6" aria-label="Mobot by the numbers">
      <div
        ref={ref}
        className="mx-auto max-w-[77rem] rounded-2xl border border-white/10 bg-white/5 p-10 md:p-12
                   shadow-[inset_-1px_1px_1px_rgba(255,255,255,0.08)]
                   grid grid-cols-2 md:grid-cols-4 gap-y-10 md:divide-x md:divide-white/10"
      >
        {stats.map((s) => (
