...
function StatItem({ target, suffix, label, active }: { target: number; suffix: string; label: string; active: boolean }) {
  ...
  return (
    <div className="text-center px-4 md:first:pl-0 md:last:pr-0">
      ...
      <span className="eyebrow block text-black/40 text-xs mt-3">{label}</span>
    </div>
  );
}
...
export default function StatCounters() {
  ...
  return (
    <section className="py-20 px-6" aria-label="Mobot by the numbers">
      <div
        ref={ref}
        className="mx-auto max-w-[77rem] rounded-2xl border border-black/10 bg-black/[0.02] p-10 md:p-12
                   grid grid-cols-2 md:grid-cols-4 gap-y-10 md:divide-x md:divide-black/10"
      >
        {stats.map((s) => (
