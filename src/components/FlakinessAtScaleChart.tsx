  ...
  return (
    <section className="py-24 px-6" aria-label="Chart: build failure probability compounds with test suite size">
      <div className="mx-auto max-w-[64rem]">
        <p className="eyebrow text-black/40 text-xs uppercase mb-4 text-center">The Cost of Noise</p>
        <h2 className="text-3xl font-bold mb-2 text-center">Why Your Builds Are Always Red</h2>
        <p className="text-black/50 text-sm text-center mb-6 max-w-[36rem] mx-auto">
          Chance at least one flaky test fails the build &mdash; even at a 0.1% per-test flake rate.
        </p>

        <div className="flex items-center justify-center gap-6 mb-8 text-xs text-black/50">
          <span className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-red-600" /> False build failure risk
          </span>
          <span className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-[#2f87c8]" /> Mobot: human-verified results
          </span>
        </div>

        <div
          className="rounded-2xl border border-black/10 bg-white p-6 md:p-10
        ...
        >
          ...
          >
            {/* gridlines */}
            {yTicks.map((t) => (
              <line key={t.label} x1={60} y1={t.y} x2={980} y2={t.y} stroke="rgba(10,10,10,0.08)" strokeWidth={1} />
            ))}
            {yTicks.map((t) => (
              <text key={t.label} x={48} y={t.y + 4} textAnchor="end" fontSize={13} fill="rgba(10,10,10,0.4)">
                {t.label}
              </text>
            ))}
            {xTicks.map((t) => (
              <text key={t.label} x={t.x} y={480} textAnchor="middle" fontSize={13} fill="rgba(10,10,10,0.4)">
                {t.label}
              </text>
            ))}
            <text
            ...
            {/* Mobot flat line */}
            <line x1={60} y1={458} x2={980} y2={458} stroke="#2f87c8" strokeWidth={2} strokeLinecap="round" />
            <circle cx={784.8} cy={458} r={5} fill="#2f87c8" stroke="#ffffff" strokeWidth={2} />
            ...
            <g>
              <rect x={560} y={90} width={380} height={112} rx={10} fill="rgba(10,10,10,0.06)" stroke="rgba(10,10,10,0.12)" />
              <text x={582} y={122} fontSize={16} fontWeight={700} fill="#dc2626">
                ~80% of builds fail erroneously
              </text>
              <text x={582} y={148} fontSize={13} fill="rgba(10,10,10,0.55)">
                at 800 test opportunities &mdash; 4 of 5 red
              </text>
              <text x={582} y={168} fontSize={13} fill="rgba(10,10,10,0.55)">
            ...
            {/* callout: Mobot */}
            <g>
              <rect x={560} y={330} width={380} height={92} rx={10} fill="rgba(10,10,10,0.06)" stroke="rgba(47,135,200,0.3)" />
              <text x={582} y={362} fontSize={16} fontWeight={700} fill="#2f87c8">
                Mobot: human-verified results
              </text>
              <text x={582} y={388} fontSize={13} fill="rgba(10,10,10,0.55)">
